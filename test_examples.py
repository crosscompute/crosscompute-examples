...
def test_add_integers(tmpdir):
    ...
    r = run(*args)
    assert r['raw_output'] == '5'
    s = serve(*args)[0]
    assert s.find(id='raw_output-meta').text.strip() == '5'


def test_find_primes(tmpdir):
    args = str(tmpdir), 'find-primes', {'x_integer': 2016}
    r = run(*args)
    assert r['raw_outputs']['unique_factor_count'] == 3


def test_count_words(tmpdir):
    args = str(tmpdir), 'count-words', {'document_text_path': 'words.txt'}
    r = run(*args)
    assert r['raw_outputs']['word_count'] == 113
    t = read_csv(r['raw_outputs']['word_count_table_path'])
